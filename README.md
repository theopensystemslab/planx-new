...
- [PNPM](https://github.com/pnpm/pnpm) `npm install -g pnpm@10.30.2`
...
5. Install all Node dependencies from the project root: `pnpm install`.
...
7. Move into the editor directory `cd apps/editor.planx.uk`.
...
- `pnpm stop` will stop your docker containers (dev or e2e) without destroying them
...
Stripe can't reach your dev machine directly, so a [Stripe CLI](https://docs.stripe.com/cli) container forwards test-mode webhooks to the API. It starts with the rest of the stack, connecting to Stripe via the test-mode `STRIPE_SECRET_KEY` from your local `.env`. 

The webhook signing secret is generated by the container and shared with the API automatically, so there's nothing to configure. See [`apps/stripe-cli`](apps/stripe-cli/README.md) for further information.
...
Make sure `pnpm` is installed globally at version 10.30.2 `pnpm add -g pnpm@10.30.2`, otherwise you may hit some unmet peer dependencies issues.
