...
2.  Then in your terminal, `cd` into this directory (from project root, `cd infrastructure/data`)

3. Log into the appropriate environment in the AWS console, so that you can follow along

4. Run through steps 1-3 under [Deploying the data layer](#steps). Before you confirm the `pulumi up` command, first make absolutely sure that you are targeting the correct stack! Then choose the `details` option, and ensure that the update Pulumi wants to perform looks something like this:
...
6. While we're waiting for that new db to become available (this takes ~7 mins on staging), we can prepare the application layer to point at the new db:

    a. In a fresh terminal, `cd` into the application layer (from project root, `cd infrastructure/application`)

    b. Run `pulumi config get db-host` - the value returned should be of the form `instance.xxx.region.rds.amazonaws.com`, where `instance` is the name of the existing db (e.g. `app5ec7032`)
  
    c. We are going to replace this value with the URI or 'endpoint' of the new db. This should be as simple as swapping that `instance` value out for the new db name (`app...`), but it's worth verifying this by clicking through into the new db in the AWS console, and comparing your construction against the `Endpoint` value given there (literally do `Ctrl + F`)

    d. Finally, run `pulumi config set --secret db-host [endpoint]`, commit the change, push, get PR approved and merge. Unlike the data layer, the application layer is deployed automatically via CI/GitHub action ([staging](https://github.com/theopensystemslab/planx-new/actions/workflows/push-main.yml) / [prod](https://github.com/theopensystemslab/planx-new/actions/workflows/push-production.yml)).
    
    Remember though, if you're restoring a db on prod, you'll need to raise a further PR for the `production` branch, get it approved and then run [this workflow](https://github.com/theopensystemslab/planx-new/actions/workflows/cron-deploy-to-production.yml). Also note that if you don't have a second dev on hand, you'll have to apply these changes to the stack directly.

7. Once the new db is 'Available', and the application is redeployed with the new `db-host` value, everything should be up and running! Go and check! `pulumi up` will continue until the old db is deleted (although it should keep a final snapshot, as per [this input](https://github.com/theopensystemslab/planx-new/blob/main/infrastructure/data/index.ts#L43)). The whole process takes less than 20 mins on staging.
