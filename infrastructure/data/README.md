...
All objects uploaded to the `user-data` bucket are scanned for malware and NSFW content (images only) by [Scanii](https://scanii.com/). This works via a trigger on the bucket which submits each object to a pair of lambdas (`Submit` and `Callback`). These (and other related resources) are manually provisioned as a [serverless application](https://serverlessrepo.aws.amazon.com/applications/arn:aws:serverlessrepo:us-east-1:484983087487:applications~UvaSoftware-Scanii-Lambda) (single CloudFormation stack) in each environment, rather than being scheduled in code and managed via Pulumi.
...
| `bucketName`                  | `user-data-xxx`                 | Differs per environment. Retrieve from the `data` layer by `pulumi stack output apiBucketId`, or check AWS console.                                                                                                          |
...
2. Copy the existing `user-data-xxx` bucket policy and keep it handy, then **extend** it with a statement denying all writes:
