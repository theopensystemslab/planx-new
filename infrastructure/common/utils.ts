...
import type { DbUrlArgs } from "./types";
...
export const createIpv4EgressRule = (
  ...
  for (const port of ports) {
    new aws.vpc.SecurityGroupEgressRule(
      `${sgName}-allow-${protocol}-ipv4-out-${port}`,
      {
        securityGroupId: securityGroupId,
        cidrIpv4: "0.0.0.0/0",
        fromPort: port,
...
export const createIpv4IngressRule = (
  ...
  for (const port of ports) {
    new aws.vpc.SecurityGroupIngressRule(
      `${sgName}-allow-${protocol}-ipv4-in-${port}`,
      {
        securityGroupId: securityGroupId,
        cidrIpv4: "0.0.0.0/0",
        fromPort: port,
...
export const createSourceSgIngressRule = (
  ...
  for (const port of ports) {
    new aws.vpc.SecurityGroupIngressRule(
      `${sgName}-allow-${protocol}-ipv4-in-${port}-from-sg`,
      {
        securityGroupId: securityGroupId,
        fromPort: port,
        toPort: port,
...
export const createDestinationSgEgressRule = (
  ...
  for (const port of ports) {
    new aws.vpc.SecurityGroupEgressRule(
      `${sgName}-allow-${protocol}-ipv4-out-${port}-to-sg`,
      {
        securityGroupId: securityGroupId,
        fromPort: port,
        toPort: port,
