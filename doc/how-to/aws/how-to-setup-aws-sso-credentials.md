...
2. Run `aws configure sso` - some of the required details can be found via [the SSO portal](https://opensystemslab.awsapps.com/start#/) in the 'Access keys' section:
...
3. Allow access from the authorisation page that should open in your browser.

4. You should be prompted for more details in the terminal:
...
5. Repeat for all environments (staging, production)

6. Test! You should be able to call commands using the CLI, for example `aws sts get-caller-identity --profile planx-staging` and get the expected result.
