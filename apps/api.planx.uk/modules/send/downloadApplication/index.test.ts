  ...
  describe("token validation", () => {
    it("fails without authorization header", async () => {
      await supertest(app)
        ...
        .expect(400)
        .then(res =>
          expect(res.body.issues[0]).toHaveProperty("message", "Authorization headers are required")
        );
    });
    ...
    it("fails without a correctly formatted authorization header", async () => {
      await supertest(app)
        ...
        .expect(400)
        .then(res =>
          expect(res.body.issues[0]).toHaveProperty("message", "Invalid token format")
        );
    });
    ...
    it("errors if matching token is not found", async () => {
      ...
      await supertest(app)
        ...
        .expect(404)
        .then(res =>
          expect(res.body.error).toEqual("INVALID_ACCESS_TOKEN")
        );
    });
    ...
    it("errors if a token is revoked", async () => {
      ...
      await supertest(app)
        ...
        .expect(410)
        .then(res =>
          expect(res.body.error).toEqual("REVOKED_ACCESS_TOKEN")
        );
    });
    ...
    it("errors if token is expired", async () => {
      ...
      await supertest(app)
        ...
        .expect(410)
        .then(res =>
          expect(res.body.error).toEqual("EXPIRED_ACCESS_TOKEN")
        );
    });
  });
  ...
  describe("payload generation", () => {
    ...
    it("throws an error if zip generation fails", async () => {
      ...
      await supertest(app)
        ...
        .expect(500)
        .then((res) => {
          expect(res.body.error).toMatch(/Failed to download application files/)
