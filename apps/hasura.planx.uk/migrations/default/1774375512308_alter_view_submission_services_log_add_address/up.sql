DROP VIEW IF EXISTS public.submission_services_log;
CREATE VIEW "public"."submission_services_log" AS
 WITH payments AS (
 ...
 SELECT ls.flow_id,
    ...
    f.team_id,
    f.name AS flow_name,
    ls.data->'passport'->'data'->'_address'->>'single_line_address' AS address
   FROM ((all_events ae
