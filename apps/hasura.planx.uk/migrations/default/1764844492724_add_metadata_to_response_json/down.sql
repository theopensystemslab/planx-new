-- Previous iteration from apps/hasura.planx.uk/migrations/default/1764778977255_v2_filter_delete_events/up.sql
CREATE
...
submissions AS (
  ...
  WHERE
    (
      -- Exclude email events
      ((se.webhook_conf) :: text !~~ '%/send-email%' :: text)
      AND (
      -- Exclude delete events
        (se.webhook_conf) :: text !~~ '%/delete-session%' :: text
      )
      AND (
