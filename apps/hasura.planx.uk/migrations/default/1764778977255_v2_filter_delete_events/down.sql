-- Previous iteration from apps/hasura.planx.uk/migrations/default/1764758889205_filter_out_delete_events/up.sql
CREATE OR REPLACE VIEW "public"."submission_services_log" AS 
 WITH payments AS (
         ...
         SELECT ((((seil.request -> 'payload'::text) -> 'payload'::text) ->> 'sessionId'::text))::uuid AS session_id,
            ...
            WHERE (
               ((se.webhook_conf)::text !~~ '%email/%'::text)
               AND ((se.webhook_conf)::text !~~ '%delete-session/%'::text) 
               AND (seil.created_at >= '2024-01-01 00:00:00+00'::timestamp with time zone)
            )
        ), all_events AS (
