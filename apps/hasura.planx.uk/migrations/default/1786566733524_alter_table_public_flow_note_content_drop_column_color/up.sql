-- drop unused column
alter table "public"."flow_note_content" drop column "color" cascade;

-- recreate functions which dropped column cascaded to
CREATE OR REPLACE FUNCTION public.rename_node_id(node_id text, replace_value text)
RETURNS text AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.remap_note_placement(placement jsonb, replace_value text)
RETURNS jsonb AS $$
DECLARE
  result jsonb;
BEGIN
...
CREATE OR REPLACE FUNCTION public.copy_flow(
  source_flow_id uuid,
  team_id integer,
  slug text,
  name text,
