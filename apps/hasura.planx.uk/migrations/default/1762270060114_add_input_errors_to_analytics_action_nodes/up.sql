...
CREATE MATERIALIZED VIEW "public"."analytics_action_nodes" AS 
 SELECT a.flow_id,
  ...
  WHERE (((al.allow_list_answers -> 'drawBoundary.action'::text) IS NOT NULL) OR ((al.allow_list_answers -> 'proposal.projectType'::text) IS NOT NULL) OR ((al.allow_list_answers -> 'rab.exitReason'::text) IS NOT NULL) OR ((al.allow_list_answers -> 'usedFOIYNPP'::text) IS NOT NULL) OR (al.has_clicked_save = true) OR (al.has_clicked_help = true) OR (al.input_errors::text != '[]'));
