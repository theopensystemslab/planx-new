...
describe("Questions and Checklists", () => {
  ...
  test("Auto-answer the option that exactly matches a passport value", () => {
    clickContinue("InitialChecklistFood", {
      answers: ["InitialChecklistOptionFruit"],
      auto: false,
    });
  ...
  test("Auto-answer the less granular option when there's a single more granular passport value and no more granular options available", () => {
    ...
    clickContinue("QuestionFruitType", {
      answers: ["OptionFruitTypeRedGrapes"],
      auto: false,
    });
    expect(computePassport()?.data).toEqual({ foods: ["fruit.grapes.red"] });

    expect(autoAnswerableOptions("QuestionFruitTypeLessGranular")).toEqual([
  ...
  test("Puts to user when we have seen this node `fn`, we do not have passport vals, and we have NOT seen all possible option `val`", () => {
    clickContinue("InitialChecklistFood", {
      answers: ["InitialChecklistOptionBlank"],
      auto: false,
    });
  ...
  test.todo(
    "Auto-answer the single most granular, left-most option when there are many matching passport values",
  );
});
...
const flow: Store.Flow = {
  ...
  QuestionFruit: {
    ...
    type: 100,
    edges: ["QuestionFruitYesOption", "QuestionFruitBlankOption"],
  },
