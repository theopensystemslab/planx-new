...
const renderUseFlowFilters = (flows: FlowSummary[] | null = mockFlows) =>
  renderHook(() => useFlowFilters({ flows, filterOptions, sortOptions }));

beforeEach(() => {
  mockSearchParams = {};
});

describe("useFlowFilters - base behaviour", () => {
  it("returns null sortedFlows when flows is null (loading state)", () => {
    const { result } = renderUseFlowFilters(null);
    expect(result.current.sortedFlows).toBeNull();
    expect(result.current.flowsHaveBeenFiltered).toBe(false);
  });

  it("returns all flows when no filters are active", () => {
    const { result } = renderUseFlowFilters();
    expect(result.current.sortedFlows).toHaveLength(mockFlows.length);
    expect(result.current.flowsHaveBeenFiltered).toBe(false);
  });

  it("sets flowsHaveBeenFiltered to true when a filter reduces the list", () => {
    mockSearchParams = { "service-status": "offline" };
    const { result } = renderUseFlowFilters();
    expect(result.current.flowsHaveBeenFiltered).toBe(true);
  });
});

describe("useFlowFilters - service-status filter", () => {
  it("returns only online services when service-status=online", () => {
    mockSearchParams = { "service-status": "online" };
    const { result } = renderUseFlowFilters();
    const slugs = result.current.sortedFlows!.map((f) => f.slug);
  ...
  it("returns only offline services when service-status=offline", () => {
    mockSearchParams = { "service-status": "offline" };
    const { result } = renderUseFlowFilters();
    expect(result.current.sortedFlows!.map((f) => f.slug)).toEqual([
  ...
  it("excludes flows that are not services even when the status matches", () => {
    ...
    mockSearchParams = { "service-status": "online" };
    const { result } = renderUseFlowFilters(flows);
    expect(result.current.sortedFlows!.map((f) => f.slug)).toEqual([
      "public-service",
    ]);
  });
...
describe("useFlowFilters - flow-type filter", () => {
  ...
  it("excludes a send-component flow that is not a service from submission", () => {
    ...
    mockSearchParams = { "flow-type": "submission" };
    const { result } = renderUseFlowFilters(flows);
    expect(result.current.sortedFlows).toHaveLength(0);
  });
...
describe("useFlowFilters - templates filter", () => {
...
describe("useFlowFilters - lps-listing filter", () => {
...
describe("useFlowFilters - multiple filters (AND behaviour)", () => {
...
describe("useFlowFilters - sorting", () => {
  ...
  it("sorts by last-published descending", () => {
    ...
    const publishedSlugs = slugs.slice(1);
