...
import { formatLastEditMessage, formatLastPublishMessage } from "../../../FlowEditor/utils";
...
export const useFlowDates = (flow: FlowSummary) => {
  ...
  const publishedDate = formatLastPublishMessage(flow.publishedFlows[0]?.publishedAt);
  ...
  return {
    displayFormatted: showPublished ? publishedDate.formatted : editedDate.formatted,
