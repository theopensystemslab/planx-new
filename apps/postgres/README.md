...
## Mirror fallback
If the live mirror fails, the build falls back to the `snapshot.debian.org` archive.

These fallback packages are only the build toolchain, which is purged from the final image, so this has no effect on the runtime.
