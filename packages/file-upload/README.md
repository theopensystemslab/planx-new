...
`apps/api.planx.uk`'s image bakes in an _injected copy_ of this package's `dist/`, pinned inside an anonymous `/api/node_modules` volume
that survives `--force-recreate`.
