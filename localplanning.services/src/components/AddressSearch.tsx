...
const AddressSearch: React.FC<Props> = ({ action }) => {
  ...
  const [matchingLpas, setMatchingLpas] = useState<LocalPlanningAuthorityFeature[]|null>(null)
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {matchingLpas?.length === 0 && <p>We couldn't find a local planning authority for this address.</p>}
      {matchingLpas && matchingLpas?.length > 0 && <p>Your local planning authorities are: {matchingLpas?.map(e => e.name)}</p>}
    </form>
  );
};
