Feature: Trial Team access
  ...
  Background:
    ...
    And there is one trial user, one standard user, and one platform admin:
      | id | first_name     | last_name   | email                       |
      | 12 | Trial          | User        | trial.user@email.com        |
      | 32 | Standard       | User        | standard.user@email.com     |  
      | 45 | Platform       | Admin       | platform.admin@email.com    |
  ...
  Scenario: I cannot turn a flow online with trial access
    ...
    But a platform admin can update the status