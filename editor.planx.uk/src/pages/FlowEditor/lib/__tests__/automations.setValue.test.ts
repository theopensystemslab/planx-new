...
describe("Auto-answering based on a SetValue", () => {
  ...
  test("Questions and Checklists auto-answer the correct paths", () => {
    setState({ flow });
    expect(upcomingCardIds()).toEqual(["SetValueChocolate", "InitialChecklistFood", "QuestionChocolate", "QuestionFruit", "QuestionRyeBread", "LastChecklistFood"]);

    // Proceed through SetValue
    clickContinue("SetValueChocolate", { data: { "food": ["chocolate"] }, auto: true });
    expect(computePassport()?.data).toHaveProperty("food");

    // Confirm all upcoming cards are auto-answerable
    expect(autoAnswerableOptions("InitialChecklistFood")).toEqual(["InitialChecklistOptionBlank"]);
    expect(autoAnswerableOptions("QuestionChocolate")).toEqual(["QuestionChocolateOptionYes"]);
    expect(autoAnswerableOptions("QuestionFruit")).toEqual(["QuestionFruitOptionBlank"]);
    expect(autoAnswerableOptions("QuestionRyeBread")).toEqual(["OptionRyeBreadBlank"]);
  ...
  test("A node using the `neverAutoAnswer` prop is not auto-answered and put to the user", () => {
    ...
    clickContinue("QuestionBreadType", { answers: ["OptionBreadTypeBagel"], auto: false });

    // Confirm all upcoming cards are auto-answerable
    expect(autoAnswerableOptions("QuestionChocolate")).toEqual(["QuestionChocolateOptionYes"]);
    expect(autoAnswerableOptions("QuestionFruit")).toEqual(["QuestionFruitOptionBlank"]);
    expect(autoAnswerableOptions("QuestionRyeBread")).toEqual(["OptionRyeBreadBlank"]);
    expect(autoAnswerableOptions("LastChecklistFood")).toEqual(["LastChecklistOptionBread", "LastChecklistOptionChocolate"]);
  });
});
