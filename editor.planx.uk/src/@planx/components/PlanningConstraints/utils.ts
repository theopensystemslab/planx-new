  ...
  if (inaccurateConstraints) {
    Object.entries(inaccurateConstraints).forEach(([inaccurateKey, inaccurateConstraint]) => {
      ...
      if (allEntitiesInaccurate) {
        intersectingConstraints[fn] = intersectingConstraints[fn].filter((intersectingKey) => !intersectingKey.startsWith(inaccurateKey));
        nots[fn].push(inaccurateKey);
      }
      ...
      if (!allEntitiesInaccurate && ["listed", "flood"].includes(inaccurateKey)) {
        ...
        const granularKeysToRemove = granularInaccurateKeys.filter((k) => !granularAccurateKeys.includes(k));
    ...
    const orphanedDesignatedKey = intersectingConstraints[fn].includes("designated") && intersectingConstraints[fn].filter((intersectingKey) => intersectingKey.startsWith("designated.")).length === 0;
