CREATE OR REPLACE VIEW "public"."analytics_summary" AS 
 SELECT a.id AS analytics_id,
    ...
    (
        SELECT ((al2.allow_list_answers -> 'user.role'::text))::text
        FROM analytics_logs al2 
        WHERE al2.analytics_id = a.id 
          AND ((al2.allow_list_answers -> 'user.role'::text))::text IS NOT NULL
          AND ((al2.allow_list_answers -> 'user.role'::text))::text != ''
