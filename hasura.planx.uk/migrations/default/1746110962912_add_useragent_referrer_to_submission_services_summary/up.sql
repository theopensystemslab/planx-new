CREATE OR REPLACE VIEW "public"."submission_services_summary" AS 
 ...
 SELECT (ls.id)::text AS session_id,
    ...
    (((ls.allow_list_answers -> 'send.analytics.userAgent'::text) -> 'os'::text) ->> 'name'::text) AS operating_system,
    (((ls.allow_list_answers -> 'send.analytics.userAgent'::text) -> 'browser'::text) ->> 'name'::text) AS browser,
    (((ls.allow_list_answers -> 'send.analytics.userAgent'::text) -> 'platform'::text) ->> 'type'::text) AS platform,
    ((ls.allow_list_answers -> 'send.analytics.referrer'::text))::text AS referrer
