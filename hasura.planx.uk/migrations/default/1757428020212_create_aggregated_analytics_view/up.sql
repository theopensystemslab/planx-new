...
FROM ((((analytics a
    ...
    LEFT JOIN LATERAL ( SELECT jsonb_array_elements_text((al.allow_list_answers -> 'proposal.projectType'::text)::jsonb) AS project_type
         WHERE (al.allow_list_answers -> 'proposal.projectType'::text IS NOT NULL)) pt ON (true))
    LEFT JOIN LATERAL ( SELECT (((al.metadata ->> 'flag'::text))::jsonb ->> 'text'::text) AS result_text
         WHERE ((al.metadata ->> 'flag'::text) IS NOT NULL)) rf ON (true)
