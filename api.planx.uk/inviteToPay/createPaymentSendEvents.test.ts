...
describe("Create payment send events webhook", () => {
  ...
  it("returns a 400 if a required value is missing", async () => {
    await supertest(app)
      ...
      .expect(400)
      .then((response) => expect(response.body.error).toEqual("Missing payload data to create payment send events"));
  });
  ...
  it("passes the correct arguments along to createScheduledEvent", async () => {
    ...
    expect(mockArgs.webhook).toBe("{{HASURA_PLANX_API_URL}}/email-submission/southwark");
  ...
  it("returns a 500 on event setup failure", async () => {
    ...
    await supertest(app)
      ...
      .expect(500)
      .then((response) => {
        expect(response.body.error).toMatch(/Failed to create payment send event/)
      });
  });
});
