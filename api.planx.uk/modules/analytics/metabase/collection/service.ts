...
/** Checks if a collection exists with name matching `councilName` exists.
 * Returns the matching collection ID if exists, otherwise false. */
export async function checkCollections(councilName: string): Promise<any> {
  try {
    console.log("Checking for collection: ", councilName);
    ...
    const matchingCollection = response.data.find((collection: any) =>
      collection.name.toLowerCase().includes(councilName.toLowerCase()),
    );
